...
const CreateBooks = () => {
  ...
  const handleSave = async (formValues: any) => {
    ...
    if (res.message === SUCCESS) {
      message.success({
        content: !bookInfo.message
          ? "Create book successfully"
          : "Create book successfully, please wait for admin to approve",
      });
  ...
  return (
    <div className={styles.container}>
      <FormBook onChange={handleGetBookInfo} />
                ...
                {fields?.map((value, index) => (
                  <div key={value.key}>
                    <div className={styles.titleWrapper}>
                      <div className={styles.titleSituation}>{`Chapter ${
                        index + 1
                      }`}</div>
